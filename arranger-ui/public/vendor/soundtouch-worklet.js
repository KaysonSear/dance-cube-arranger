/* AUTO-DERIVED from vendor/soundtouch.js (SoundTouch JS, LGPL 2.1 — see that file's
 * header for copyright). DSP core (unchanged) + an AudioWorklet processor so the
 * time-stretch runs off the main thread (fixes slow-playback '抽帧' dropouts).
 * DO NOT hand-edit; regenerate by re-slicing soundtouch.js. */
...
// --- Interleaved source over transferred channel data (worklet has no AudioBuffer). ---
// Mirrors WebAudioBufferSource.extract() so SimpleFilter can drive it unchanged.
class RawBufferSource {
  constructor(left, right) {
    this._left = left;
    this._right = right && right.length ? right : left;
    this._position = 0;
  }
  get dualChannel() {
...
// Runs SoundTouch on the audio render thread. Messages in: buffer / tempo / rate /
// pitch / seek. Messages out: position (throttled) / end.
class SoundTouchProcessor extends AudioWorkletProcessor {
  constructor() {
    ...
    this._ended = false;
    this._report = 0;
    // 断连(pause)时 process() 仍会被调用(返回 true = actively processing),若不闸住就会继续
    // 消耗源音频 —— 实测暂停 3s 源位置前进 3.07s,并可跑到曲尾误发 end。默认 true 保持原行为。
    this._running = true;
  ...
  _onMessage(d) {
    if (!d) return;
    if (d.type === 'buffer') {
      const source = new RawBufferSource(d.left, d.right);
      this._filter = new SimpleFilter(source, this._soundtouch, () => {});
      this._ended = false;
    } else if (d.type === 'tempo') {
      this._soundtouch.tempo = d.value;
    } else if (d.type === 'rate') {
      this._soundtouch.rate = d.value;
    } else if (d.type === 'pitch') {
      this._soundtouch.pitch = d.value;
    } else if (d.type === 'seek') {
      if (this._filter) {
        this._filter.sourcePosition = d.sourcePosition;
        this._ended = false;
      }
    } else if (d.type === 'running') {
      this._running = !!d.value;
    }
  }
  process(inputs, outputs) {
    ...
    if (!this._filter || this._ended || !this._running) {
      // 未 running 时输出静音且**不调用 extract**,从而不消耗源(消除暂停期泄漏与 CPU 空转)
      left.fill(0);
      if (right) right.fill(0);
      return true;
    }
    if (this._samples.length < frames * 2) this._samples = new Float32Array(frames * 2);
    const extracted = this._filter.extract(this._samples, frames);
    if (extracted === 0) {
      this._ended = true;
      left.fill(0);
      if (right) right.fill(0);
    ...
    let i = 0;
    for (; i < extracted; i++) {
      left[i] = this._samples[i * 2];
      if (right) right[i] = this._samples[i * 2 + 1];
    }
    for (; i < frames; i++) {
      left[i] = 0;
      if (right) right[i] = 0;
